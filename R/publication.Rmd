...
ggplot(tCS_data[hmp_type == "blue_green" & patid %in% c(1, 75), ], aes(x = hmp_x, y = hmp_y)) +
  geom_line(aes(group = factor(date_of_exam), color = factor(date_of_exam))) +
  facet_grid(patid ~ factor(frequency))

ggsave(here("images", "drift.png"))
